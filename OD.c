...
double condition(double *A){
    ...
    info = LAPACKE_dgesvd(LAPACK_ROW_MAJOR, 'N', 'N', JJ, JJ, A, JJ, SV, NULL, JJ, NULL, JJ, work);
    if (info!=0) exit(0);
...
int main(int argc, char *argv[]){
    ...
    int OD_stepsize_fraction=1e3, OD_partition_bool=0, OD_AS=0, OD_RS=0;
    double OD_stepsize=t_F/(double)OD_stepsize_fraction, OD_position=t;
    double OD_TOL = 1e7, OD_MINTOL = 1e2, OD_MAXTOL = 1e10;
    double OD_TOL_inv=1.0/OD_TOL, OD_MINTOL_inv=1.0/OD_MINTOL, OD_MAXTOL_inv=1.0/OD_MAXTOL;
    double OD_last_t = t, OD_last_stepsize=step_size, OD_temp_double, OD_max_magnitude, OD_min_magnitude;
    ...
    while(t<t_F){
        ...
        if (OD_partition_bool){
            ...
            if (OD_max_magnitude>OD_MAXTOL || OD_min_magnitude<OD_MAXTOL_inv){
                ...
                OD_position = OD_last_t;

                OD_stepsize*=0.8;
                if (OD_stepsize<step_size) OD_stepsize=2*step_size;
                OD_RS++;

                printf("OD deceleration (%d) --> OD_stepsize=%le\n", OD_RS, OD_stepsize);
                continue;
            }        
            ...
            if (OD_max_magnitude>OD_TOL || OD_min_magnitude<OD_TOL_inv){
                ...
                for (jj=0; jj<JJ; jj++){
                    for (cc=0; cc<CC; cc++)  MY_JET_DATA(OD_last_jet[jj], cc) = MY_JET_DATA(tmp_2[jj], cc); 
                    for (cc=0; cc<JJ; cc++){
            ...
            printf("OD position=%.15lf (num %d), t=%.15lf, step_size=%le\n", OD_position, od, t, step_size);

            if (OD_max_magnitude<OD_MINTOL && OD_min_magnitude>OD_MINTOL_inv){
                OD_stepsize*=1.1;
                if (OD_stepsize>t_F-t) OD_stepsize=t_F-t;
                OD_AS++;

                printf("OD acceleration (num %d) --> OD_stepsize=%le\n", OD_AS, OD_stepsize);
            }
    ...
    printf("Num of OD:%d, with %d accelerations and %d decelerations\n", od, OD_AS, OD_RS);
